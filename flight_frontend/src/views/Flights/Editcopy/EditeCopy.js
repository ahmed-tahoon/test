...
import AddFlightCard from './EditeCopyCard'
...
const {GetOnecountryData} =useSelector(state => state.GetOnecountryRedux)
        ...
        <option selected disabled>Open this select menu</option>

            {
                GetOnecountryData?.city?.map((item,index)=>{return(
                 <option selected={item?.name === customerData?.city?.name ? true : false } value={`${item?.id}-${item.name}`}>{item?.name}</option>
   ...
   control: (baseStyles, state) => ({
     ...
     width:"100%"
        ...
        <option selected={false} disabled>Open this select menu</option>

            {
                GetcountryData?.map((item,index)=>{return(
                 <option selected={item?.name === customerData?.country?.name ? true : false }  value={item?.id}>{item?.name}</option>
        ...
        InputWight:( <>
            <input value={weight} onChange={Hanadelweight} style={{borderRadius:"10px", backgroundColor:COLORS.blue,width:"100%"}} className="form-control" type="number" placeholder="Wight" aria-label="default input example"/>

        </>),
         InputFightNumber:( <>
            <input value={flight_number} onChange={Hanadelflight_number} style={{borderRadius:"10px", backgroundColor:COLORS.blue,width:"100%"}} className="form-control" type="text" placeholder="Fight No." aria-label="default input example"/>

        </>),
        InputArrivalTime:( <>
            <input value={arrival_time} onChange={Hanadelarrival_time} style={{borderRadius:"10px", backgroundColor:COLORS.blue,width:"100%"}} className="form-control" type="time" placeholder="Arrival Time" aria-label="default input example"/>

        </>),
        InputDepatureTime:( <>
            <input value={departure_time} onChange={Hanadeldeparture_time} style={{borderRadius:"10px", backgroundColor:COLORS.blue,width:"100%"}} className="form-control" type="time" placeholder="Depature Time" aria-label="default input example"/>

        </>),
        InputDurationTime:( <>
            <input value={duration} onChange={Hanadelduration} style={{borderRadius:"10px", backgroundColor:COLORS.blue,width:"100%"}} className="form-control" type="number" placeholder="Duration" aria-label="default input example"/>
