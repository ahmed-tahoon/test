...
const CityFilter = ({title,tableData,settableData}) => {
...
const handelFilter = ()=>{
  ...
  if(title === "AirLines"  ){
    if(checked && GetflightCompanyData && checkedItems ){
      ...
      for (let i = 0; i < checkedItems.length ; i++ ){
        const updatecity = tableData?.filter((item)=>{return(
          item?.company?.name === checkedItems[i]
    ...
    if(ceckedsortAtoZ){
      ...
      update.sort((b, a) => b.company.name.localeCompare(a.company.name));
      update.reverse()
      settableData(update)
      setcheckData(update)
  ...
  if( title === "Dep.APT"){
    if(checked && GetdapartureData && checkedItems ){
      ...
      for (let i = 0; i < checkedItems.length ; i++ ){
        const update = tableData?.filter((item)=>{return(
          item?.departure_airport?.name === checkedItems[i]
    ...
    if(ceckedsortZtoA){
      ...
      update.sort((a, b) => a.departure_airport?.name.localeCompare(b.departure_airport?.name));
      settableData(update)
      setcheckData(update)
    ...
    if(ceckedsortAtoZ){
      ...
      update.sort((b, a) => b.departure_airport?.name.localeCompare(a.departure_airport?.name));
      update.reverse()
      settableData(update)
      setcheckData(update)
